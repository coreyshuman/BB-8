 ...
 * Corey Shuman         8/22/16     Added blocking TickDelay for compatibility
********************************************************************/
#define __TICK_C
...
/*****************************************************************************
  Function:
	void TickDelay(DWORD ms)

  Summary:
        Function blocks until the time in milliseconds has elapsed
...
void TickDelay(DWORD ms) {
    DWORD tick = TickGet();
    while(TickGet() - tick < ms*TICK_SECOND/1000 );
    return;
}
